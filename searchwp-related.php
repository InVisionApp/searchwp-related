...
Text Domain: searchwp_related
Domain Path: languages

Copyright 2017-2018 SearchWP, LLC
...
define( 'SEARCHWP_RELATED_VERSION',     '1.1.1' );
...
function searchwp_related_below_php_version_notice() {
	...
	echo '<div class="error"><p>' . esc_html( sprintf( __( 'Your version of PHP (%s) is below the minimum version of PHP required by SearchWP Related (5.3). Please contact your host and request that your version be upgraded to 5.3 or later.', 'searchwp_related' ), PHP_VERSION ) ) . '</p></div>';
}
...
function searchwp_related_update_check() {
	...
	if ( ! defined( 'SEARCHWP_RELATED_VERSION' ) ) {
		return false;
	}
...
function searchwp_related_load_textdomain() {
	 ...
	 */
	$locale        = apply_filters( 'plugin_locale', $get_locale, 'searchwp_related' );
	$mofile        = sprintf( '%1$s-%2$s.mo', 'searchwp_related', $locale );
	...
	if ( file_exists( $mofile_global ) ) {
		...
		load_textdomain( 'searchwp_related', $mofile_global );
	} elseif ( file_exists( $mofile_local ) ) {
		// Look in local /wp-content/plugins/searchwp-related/languages/ folder
		load_textdomain( 'searchwp_related', $mofile_local );
	} else {
		// Load the default language files
		load_plugin_textdomain( 'searchwp_related', false, $lang_dir );
	}
}
